const getBaseUrl = () => {
    // Si estamos en localhost usar el puerto del backend, sino usar la misma URL (para proxy o LoadBalancer)
...
document.addEventListener('DOMContentLoaded', () => {
    const apiUrl = getBaseUrl() + '/api/entregas';
    let allDeliveries = [];
    ...
    document.getElementById('view-facturas-btn').onclick = async () => {
        const content = document.getElementById('drawer-content');
        document.querySelector('#history-drawer h2').textContent = 'Facturas (Externo)';
        content.innerHTML = '<p>Cargando facturas desde el microservicio remoto...</p>';
        drawer.classList.remove('hidden');
        drawerBackdrop.classList.remove('hidden');
        ...
        try {
            ...
            if (!Array.isArray(facturas) || facturas.length === 0) {
                content.innerHTML = '<p>No se encontraron facturas en el sistema externo.</p>';
                return;
            }

            let html = '<div class="facturas-list">';
            facturas.forEach(f => {
                html += `
                    <div class="factura-item">
                        <h3>Factura #${f.id || 'N/A'}</h3>
                        <p><strong>Total:</strong> $${f.total || '0.00'}</p>
                        <p><strong>Cliente:</strong> ${f.cliente || 'Desconocido'}</p>
                    </div>
                `;
            });
            html += '</div>';
            ...
            content.innerHTML = `<p style="color:red">Error al conectar con el microservicio de Facturación: ${e.message}</p>`;
        }
    };
